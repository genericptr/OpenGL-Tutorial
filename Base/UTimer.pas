...
type
	// subclass TTimerDispatcher for TWorldTimerDispatcher
	// make another schedule function which for world timers
	// push/pop TTimerDispatcher in schedules
	// override TTimerDispatcher.Process
	// exit if WorldPaused is set
	TTimer = class;
...
begin
	result := TTimer.Create(_interval, _action, _target, _argument, _repeats);
	result.Install;
	result.Release;
end;
...
begin
	result := TTimer.Create(_interval, _invocation, _repeats);
	result.Install;
	result.Release;
end;
...
begin	
	...
	if in_dispatcher = nil then
		dispatcher := TTimerDispatcher(GlobalDispatchers.GetValue(0))
	else
